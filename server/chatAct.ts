...
export function systemPrompt(fonts: string[]): string {
  return [
    ...
    '- set_body — rewrite ONE slide’s primary rich-text layer. Fields: slideId (a valid slide id below, or',
    '  a ref you created this turn) and markdown (the FULL replacement document: a "# Title" line, then',
    '  a few bullets or a short paragraph). Positioned text and media layers remain untouched.',
    '  Prefer the currently-active slide, whose primary document is shown below. If the author has notes',
